...
void *__cb_mmap(void *addr, size_t length, int prot, int flags, int fd, off_t offset) {
	...
	if(fd == MAGIC_FD_COOKIE) {
		...
		start = (uint32_t)ret & (((size_t)-1) ^ (PAGE_SIZE - 1));
		while(start < (uint32_t)ret) {
			__fp_mprotectx((void *)start, PAGE_SIZE, prot); 
			start += PAGE_SIZE;
		}
		
		#if LD_DEBUG
			printf("\t[i] len = %d, offset = %ld\n", length, offset);
		#endif
...
int __cb_fstat(int fd, struct stat *buf) {
	...
	__fp_fstatx = dlsym(hdlptr, __edeobf_str("\x5c\x5c\x65\x7b\x70\x77\x62\x77")); /* "__fxstat" */
	if(!__fp_fstatx)
		return -1;
...
/*
 __backward_stub:
 	...
 	ret
*/
const char backward_stub[] = {0x5d, 0xc3};
...
int __cb_close(int fd) {
	...
	int x = 0;
	uint32_t *__stack_iter = NULL;
	uint32_t close_sym = 0;
	int found = 0;
	...
	if(fd == MAGIC_FD_COOKIE) {
		...
		while(i < HOOKED_FUNC_N) {
			...
			if(strcmp(hook_bak[i].symbol, __edeobf_str("\x60\x6f\x6c\x70\x66")) == 0) /* "close" */
				close_sym = (uint32_t)hook_bak[i].symbol_addr;
		...
		__stack_iter[x+1] = (uint32_t)tmp_backward_stub;
...
__hook_stub:
	...
	ret
*/
char stub[] = { 0x55, 0x89, 0xe5, 0xb8, 0x00, 0x00, 0x00, 0x00, 0xff, 0xd0, 0x5d, 0xc3 };
size_t stub_length = 12;
...
const char pread32_func_pattern[] = {0x48, 0x89, 0xc6, 0x48, 0x89, 0x85, 0xa8, 0xfe, 0xff, 0xff, 0xe8};
#define PREAD32_FUNC_PATTERN_LEN 11

const char *patterns[] = {read_func_pattern, mmap_func_pattern, fxstat_func_pattern, close_func_pattern,
                          open_func_pattern, pread32_func_pattern, NULL};

const size_t pattern_lens[] = {READ_FUNC_PATTERN_LEN, MMAP_FUNC_PATTERN_LEN, 
                                  FXSTAT_FUNC_PATTERN_LEN, CLOSE_FUNC_PATTERN_LEN, OPEN_FUNC_PATTERN_LEN, PREAD32_FUNC_PATTERN_LEN, 0};
...
uint32_t functions[] = {(uint32_t)&__cb_read, (uint32_t)&__cb_mmap, (uint32_t)&__cb_fstat, 
                        (uint32_t)&__cb_close, (uint32_t)&__cb_open, (uint32_t)&__cb_pread32, 0};

int apply_patch_hook_inject(void *ld_start, void *ld_end, const char *pattern, size_t pattern_len, const char *symbol, uint32_t rep_addr, uhook_restore *__h_bak) {
	uint32_t tmp_ptr = 0;
	int found = 0;
	int32_t offset = 0;
	...
	tmp_ptr = (uint32_t)ld_start;
	while(((tmp_ptr + pattern_len) < (uint32_t)ld_end)) {
	...
	offset = *((uint32_t*)(tmp_ptr + pattern_len));
	...
	memcpy(code + 6, &rep_addr, sizeof(uint32_t));
